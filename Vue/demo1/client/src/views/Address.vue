<template>
  <div>
    <HeadNav></HeadNav>
    <NavBread><span>地址</span></NavBread>
      ...
      <div class="container">
        <div class="checkout-addr">
              ...
              <ul>
                ...
                <li v-for="(item,index) in addressList" :key="index" :class="{'check':checkIndex == index}" @click="checkIndex = index;selectedAddrId = item.addressId;">
                  ...
                  <div class="addr-opration addr-set-default">
                    <a v-if="!item.isDefault" href="javascript:;" @click="setDefault(item.addressId)" class="addr-set-default-btn"><i>设置默认地址</i></a>
                  </div>
          ...
          <div class="lemall-msg-info hidden">
            <span>The region you selected is not within our delivery area. Please select another shipping address
              within our delivery areas.</span>
          </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      //init 初始化 。。。。
      init(){
        this.$http.get('/api/users/addressList').then((result) => {
